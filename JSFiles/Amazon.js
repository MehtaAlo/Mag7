...
const mockFundamentals = {
    peRatio: (40 + Math.random() * 15).toFixed(1),                  // Amazon-like placeholder
    marketCap: Math.floor(1600000000000 + Math.random() * 300000000000), // ~$1.6T–$1.9T
...
async function getAMZNQuote() {
    ...
    try {
        ...
        console.error("Error fetching AMZN quote:", error);
...
async function getAMZNHistory() {
    ...
    try {
        ...
        console.error("Error fetching AMZN history:", error);
        createChart(["N/A"], [0]);
    }
}
...
function createChart(labels, data) {
    let ctx = document.getElementById("amznChart").getContext("2d");
...
// --- INITIALIZATION ---
getAMZNQuote();
setInterval(getAMZNQuote, 30000);
getAMZNHistory();